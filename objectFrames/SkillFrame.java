...
public class SkillFrame extends GameObject {

	Handler handler;

	private Image skill;

	private int cooldown; //in ms --> 1000 = 1 second. Assigned in the constructor unless unaffected by levels.
	private int level;
	
	private static String skillName;
	private static String[] skillDescription = new String[3]; //determined in skillDescription method
	private static int skillID = 000;
	...
	private static float level1Cooldown;
	private static float level2Cooldown;
	private static float level3Cooldown;
	private static float resultCooldown;
	

	private long timeNow; //current system time assigned in tick();

	public SkillFrame(float x, float y, Handler handler, ID Id, int level) {
		super(x, y, Id);
