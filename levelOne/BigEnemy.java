...
public class BigEnemy extends GameObject{
	...
	public BigEnemy(float x, float y, float velX, float velY, ID id, Handler handler) {
	...
	public void tick() {
		...
		if(shootTimer <= 0 && moveTimer <= 0){
			...
			float a = (float) (x + Math.sin(-theta)*400);
			float b = (float) (x + Math.sin(theta)*400);
			handler.addObject(new BigEnemyBullet (a+25, y+50, 0, 2, ID.EnemyBullet, handler));
