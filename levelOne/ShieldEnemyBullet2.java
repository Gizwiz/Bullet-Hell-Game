...
public class ShieldEnemyBullet2 extends GameObject{
	
	private Handler handler;
	private Image bullet, bulletFaded;
	
	public ShieldEnemyBullet2(float x, float y, float velX, float velY, ID id, Handler handler) {
		super(x, y, velX, velY, id);
		this.handler = handler;
		this.velX = velX;
		this.velY = velY;
		try {
			bullet = ImageIO.read(new File("bin/used/shieldEnemyBullet2.png"));
			bulletFaded = ImageIO.read(new File("bin/used/shieldEnemyBulletFaded2.png"));
		} catch (IOException e) {
