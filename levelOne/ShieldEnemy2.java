...
public class ShieldEnemy2 extends GameObject{
	...
	public ShieldEnemy2(float x, float y, float velX, float velY, ID id, Handler handler) {
		...
		try{
		shield = ImageIO.read(new File("bin/used/enemyMagicCircle2.png"));
		}
		catch (IOException e){
			e.printStackTrace();
		}
	...
	public void tick() {
		...
		//BIG BULLET
		if (timer == 0){
			handler.addObject(new ShieldEnemyBullet2((int)x-25, (int)y+25, 0, 4, ID.EnemyBullet, handler));
		timer = 175;
		}
		//SMALL BULLET
		if(timer2 == 0){
			handler.addObject(new ShieldEnemyBulletSmall2((int)x, (int)y, 0, 2, ID.EnemyBullet, handler));
			handler.addObject(new ShieldEnemyBulletSmall2((int)x, (int)y, 1, 2, ID.EnemyBullet, handler));
			handler.addObject(new ShieldEnemyBulletSmall2((int)x, (int)y, -1, 2, ID.EnemyBullet, handler));
		timer2 = 125;
		}
