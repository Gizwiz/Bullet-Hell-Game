...
public class ShieldEnemyBulletSmall2 extends GameObject{
	
	private Image bullet;
	private Handler handler;

	public ShieldEnemyBulletSmall2(float x, float y, float velX, float velY, ID id, Handler handler) {
		super(x, y, velX, velY, id);
		this.handler = handler;
		this.velX = velX;
		this.velY = velY;
		try {
			bullet = ImageIO.read(new File("bin/used/shieldEnemyBulletSmall.png"));
		} catch (IOException e) {
	...
	public void render(Graphics2D g2d) {
		g2d.drawImage(bullet, (int)x, (int)y, 32, 32, null);
		if(MainSettings.showHitBoxes){
		g2d.setColor(Color.red);
			g2d.drawRect((int)x+11,(int)y+11, 8, 8);
		}
	}
