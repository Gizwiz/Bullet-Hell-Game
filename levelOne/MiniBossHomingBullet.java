...
public class MiniBossHomingBullet extends GameObject{
	...
	public MiniBossHomingBullet(float x, float y, float velX, float velY, ID id, Handler handler) {
		super(x, y, velX, velY, id);
		this.handler = handler;
		try {
			bullet = ImageIO.read(new File("bin/used/mbossHomingBullet.png"));
		} catch (IOException e) {
		...
		if(diffX < 0){
			moveX = (float) -(diffX/80.0);
		}
		if(diffX > 0){
			moveX = (float) -(diffX/80.0);
		}
		if(diffY < 0){
			moveY = (float) -(diffY/80.0);
		}
		if(diffY > 0){
			moveY = (float) (diffY/80.0);
		}
	...
	public Rectangle getBounds(){
		return new Rectangle((int)x+77,(int)y+73, 45, 45);
	}
	...
	public void render(Graphics2D g2d) {
		g2d.drawImage(bullet, (int)x, (int)y, 200, 200, null);
		if(MainSettings.showHitBoxes){
		g2d.setColor(Color.red);
			g2d.drawRect((int)x+77,(int)y+73, 45, 45);
		}
	}
